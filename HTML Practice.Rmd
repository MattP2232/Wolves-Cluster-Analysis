...
<script>
   $(document).ready(function() {
     $head = $('#header');
     $head.prepend('<img src=\"Wolves.png\" style=\"float: right;width: 150px;\"/>')
   });
</script>
...
# K-Means Clustering: Calculations and steps for clustering our data
...
Below is our complete set of unique Timberwolves players who have logged at least 500 minutes in a season since the 2012.
...
Before we develop our k means algorithm, we must determine the optimal number of clusters for analysis. In doing so, we will calculate our clustering algorithm at a specific value of k, with k being the number of clusters, and determine the within cluster variation at each value of k. We’ll then plot each within cluster variation value and determine the optimal number of clusters using this plot. Usually, we choose the number of clusters where the decrease in within cluster variation is minimal or there is a noticeable elbow in the plot. Below is our plot for cluster optimization, where I ultimately choose nine as the ideal number of clusters.
...
Another note to keep in mind when scrolling through the plots is on turnover percentage. A low turnover percentage indicates a player commits very few turnovers as a percent of total turnovers committed by players on the court. Therefore, a cluster center for TO% below zero would be a positive for a cluster group. This is the only cluster center where a value below zero would be a positive when quantifying player performance.
...
for(i in seq_along(cluster)) {
  ...
  cn <- cluster_names[i]
  #img <- readPNG(ip[i])
  #img_grob <- rasterGrob(img, interpolate = TRUE, width = unit(1.25, "inches"))
...
Below are the visuals of all 9 clusters, labeled by what I determined to be a proper group name given their cluster centers and the players present (but it’s up for interpretation!). In addition, I'll provide the initial function to arrange the first ggplot with the table of players present in the cluster, but hide the other eight to avoid repetition.
...
From there, I calculated the sum of square differences between Monte’s numbers and the cluster centers within each cluster. Below is a table containing these sum of square differences between Monte and all nine clusters.
...
suppressWarnings({
  ricky_monte <- km_centers %>% 
    ...
    geom_point(data = long_data, aes(x = x, y = y), color = "#253494") +
    ...
    ##annotation_custom(img_grob_3, xmin = Inf, xmax = 4, ymin = Inf, ymax = 2.04) +
    coord_cartesian(ylim = c(-3, 3), clip = "off") 
})

cluster_grids <- grid.arrange(ricky_monte, combined_table, layout_matrix = rbind(c(1, 3)))
```
